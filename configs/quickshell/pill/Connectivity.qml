...
/**
 * 繋 CONNECTIVITY surface: connectivity rows (auto-detected networks, Bluetooth)
 * with WLAN and Bluetooth drill-in subviews that cross-fade in place.
 * Derived from original Link surface.
 */
PillSurface {
    id: root
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                root.warpOn = this.text.includes("Status update: Connected");
            }
        }
    }
    ...
    Timer {
        ...
        repeat: true
        triggeredOnStart: true
        onTriggered: warpStatusProc.running = true
    }
            ...
            Item {
                ...
                Row {
                    ...
                    IconChip {
                        ...
                        glyph: "bolt"
                        on: root.warpOn
                        tipTitle: "Cloudflare WARP"
                        tipDesc: root.warpOn ? "Connected" : "Disconnected"
